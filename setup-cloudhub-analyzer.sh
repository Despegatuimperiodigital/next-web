...
# Función para crear archivo con contenido si no existe
create_file_if_not_exists() {
    ...
    if [ ! -f "$file_path" ]; then
        ...
        echo -e "${GREEN}Creado archivo: ${BLUE}$file_path${NC}"
    else
        echo -e "${BLUE}El archivo ya existe: ${file_path}${NC}"
    fi
}

# Verificar que estamos en un proyecto Next.js
if [ ! -f "package.json" ] || ! grep -q "next" "package.json"; then
    echo -e "${RED}Error: Este script debe ejecutarse en la raíz de un proyecto Next.js${NC}"
    exit 1
fi

echo -e "${GREEN}Integrando analizador de rendimiento...${NC}"

# Crear el analizador principal
create_file_if_not_exists "lib/ChilePerformanceAnalyzer.js" '
...
# Crear la ruta API
create_file_if_not_exists "app/api/analyze/route.js" '
