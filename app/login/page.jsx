...
export default function IniciarSesion() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await signIn('credentials', {
      email,
      password,
      redirect: false, // No redirige automáticamente, permite manejarlo
    });
    ...
    if (result?.error) {
      ...
      // Redirigir al dashboard después de un inicio exitoso
      router.push('/dashboard');
    }
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                />
                <label htmlFor="remember-me" className="ml-2 block text-sm text-secondary-dark">
                  Recuérdame
                </label>
              </div>

              <div className="text-sm">
                <Link href="/recuperar-contrasena" className="font-medium text-primary hover:text-primary-light">
            ...
            <div>
              <button
                type="submit"
                className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-primary-foreground bg-primary hover:bg-primary-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
              >
                Iniciar sesión
              </button>
            </div>
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-card text-secondary-dark">
                  O continúa con
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              <div>
                <a
                  href="#"
                  className="w-full inline-flex justify-center py-2 px-4 border border-input rounded-md shadow-sm bg-card text-sm font-medium text-secondary-dark hover:bg-muted"
                >
